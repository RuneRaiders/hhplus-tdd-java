...
public class PointConcurrentTest {
    ...
    * */
    ...
    class 동시성_제어_사용 {
        ...
        @DisplayName("10개의 스레드를 생성하여 500포인트를 가진 계정이 10Point를 10번 사용할 경우 400포인트가 된다.")
        @Test
        void 단일_계정_포인트_사용_간_동시성_제어_테스트() throws InterruptedException{
            ...
            pointService.charge(ANY_ID, balance);   // 포인트 충전 : 500L
            ...
            for(int i = 0; i < taskCount; i++){
                ...
                executorService.submit(() -> {
                    try {
                        log.info("START {} ", index);
                        pointService.use(ANY_ID, usedPoint); // ANY_POINT : 100 포인트를 10번 충전
                        log.info("END {} ", index);
                    } finally {
                        countDownLatch.countDown();             // 작업 완료 알림
                        log.info("latch count {} ", countDownLatch.getCount());
            ...
            assertAll(
                () -> assertThat(balance - usedPoint * taskCount).isEqualTo(resultUserPoint.point()),
                () -> assertThat(pointHistoryList.size()).isEqualTo(taskCount + 1)              // 10 + charge history
            );
        }

        @DisplayName("10개의 스레드를 생성하여 500포인트를 가진 10개의 계정이 10Point를 사용한다.")
        @Test
        void 다중_계정_포인트_사용_간_동시성_제어_테스트() throws InterruptedException{
            ...
            for(long userId : users){
                pointService.charge(userId, balance);
            }
            ...
            for(long userId : users){
                executorService.submit(() -> {
                    ...
                    try {
                        barrier.await();                        // 모든 스레드들이 대기 상태에 빠짐
                        log.info("START {} ", userId);
                        pointService.use(userId, usedPoint); // ANY_POINT : 100 포인트
                        log.info("END {} ", userId);
                    } catch (Exception e) {
                        log.error(e.getMessage());
                    } finally {
            ...
            for(long userId : users){
                ...
                log.info("userId={} point={} history={}", userId, userPoint.point(), pointHistoryList);     // 각 정보가 잘 들어갔는지 확인하기 위한 로그 조회
                assertAll(
                    () -> assertThat(userPoint.point()).isEqualTo(balance - usedPoint),
                    () -> assertThat(pointHistoryList.size()).isEqualTo(1 + 1)                     // charge history 1 + use history 1
                );
            }
        }
    }
